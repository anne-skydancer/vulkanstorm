...
void LLVolumeLODGroup::accumulateSourceMemory(U64& arrays, U64& volumes, U64& faces, U64& octrees) const
{
    for (const auto& volume : mVolumeLODs)
    {
        if (volume.isNull() || !volume->isMeshAssetLoaded()) continue;
        ++volumes;
        for (S32 i = 0; i < volume->getNumVolumeFaces(); ++i)
        {
            const auto& face = volume->getVolumeFace(i);
            ++faces;
            // Positions/normals/UVs share one allocation. Do not count each pointer.
            const U64 count = U64(llmax(0, face.mNumAllocatedVertices));
            if (face.mPositions) arrays += count * 32 + ((count * 8 + 15) & ~U64(15));
            const U64 used = U64(llmax(0, face.mNumVertices));
            if (face.mTangents) arrays += used * 16;
            if (face.mWeights) arrays += used * 16;
            if (face.mIndices) arrays += ((U64(llmax(0, face.mNumIndices)) * 2 + 15) & ~U64(15));
            if (face.getOctree()) ++octrees;
        }
    }
}

void LLVolumeMgr::sampleSourceMemory()
{
    if (mDataMutex) mDataMutex->lock();
    auto it = mMemorySampling ? mVolumeLODGroups.upper_bound(&mMemoryCursor) : mVolumeLODGroups.begin();
    ++mMemorySlices;
    for (U32 scanned = 0; it != mVolumeLODGroups.end() && scanned < 64; ++scanned, ++it)
    {
        mMemoryCursor = *it->first;
        mMemorySampling = true;
        ++mMemoryGroups;
        it->second->accumulateSourceMemory(mMemoryArrays, mMemoryVolumes, mMemoryFaces, mMemoryOctrees);
    }
    const bool complete = it == mVolumeLODGroups.end();
    if (mDataMutex) mDataMutex->unlock();
    if (complete)
    {
        LL_INFOS("SourceMeshMemory") << "managed_array_estimate_bytes=" << mMemoryArrays
            << " loaded_lods=" << mMemoryVolumes << " faces=" << mMemoryFaces
            << " faces_with_octree=" << mMemoryOctrees << " groups_visited=" << mMemoryGroups
            << " sweep_slices=" << mMemorySlices << " nominal_slice_seconds=5" << LL_ENDL;
        mMemorySampling = false;
        mMemoryArrays = mMemoryVolumes = mMemoryFaces = mMemoryOctrees = mMemoryGroups = 0;
        mMemorySlices = 0;
    }
}
