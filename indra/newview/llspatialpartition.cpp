...
void renderPhysicsShape(LLDrawable* drawable, LLVOVolume* volume, bool wireframe)
{
    ...
    else if (type == LLPhysicsShapeBuilderUtil::PhysicsShapeSpecification::PRIM_CONVEX)
    {
        ...
        if (phys_volume->mHullPoints && phys_volume->mHullIndices)
        {
            ...
            gGL.diffuseColor4fv(color.mV);
            // Uses the renderer's buffered generic attributes, including in Core contexts.
            LLVertexBuffer::drawElements(LLRender::TRIANGLES, phys_volume->mHullPoints,
                nullptr, phys_volume->mNumHullIndices, phys_volume->mHullIndices);
        }
        else
        {
            gGL.diffuseColor3f(1,0,1);
